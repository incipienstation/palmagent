...
export const SubmitMessageSchema = z.object({
  ...
  text: z.string().trim().min(1).max(100_000), images: z.array(ImageAttachmentSchema).optional(),
...
export const MessageActionSchema = z.discriminatedUnion("action", [
  ...
  z.object({ action: z.literal("save"), token: z.string().uuid(), version: z.number().int(), text: z.string().trim().min(1).max(100_000), images: z.array(ImageAttachmentSchema).optional() }),
