...
try {
  ...
  const searchPaths = join(scratch, "search folders");
  mkdirSync(join(searchPaths, "project", ".git"), { recursive: true });
  const searchSettings = (...args) => JSON.parse(execFileSync(bin, ["settings", ...args, "--json"], { cwd: scratch, env, encoding: "utf8" }));
  assert.deepEqual(searchSettings("get").repoRoots, []);
  searchSettings("add", "repo-roots", searchPaths, "--dry-run");
  assert(!existsSync(join(scratch, "data/settings.json")), "settings dry run does not write");
  searchSettings("add", "repo-roots", searchPaths);
  const settingsResponse = await fetch(origin + "/api/settings/repos");
  assert.equal(settingsResponse.headers.get("cache-control"), "no-store");
  assert.deepEqual((await settingsResponse.json()).repoRoots, [searchPaths]);
  assert.deepEqual((await (await fetch(origin + "/api/repos/discover")).json()).repos.map((repo) => repo.path), [join(searchPaths, "project")]);
  searchSettings("set", "repo-roots");
  assert.deepEqual((await (await fetch(origin + "/api/repos/discover")).json()).repos, []);
  searchSettings("reset", "repo-roots");
  assert.equal(searchSettings("get").source, "installation");
  server.assertRunning();
