...
import { extractOutputImages } from "../src/application/output-images.js";
...
test("handoff persists ownership; socket dispatch previews a live writer and transfers exactly once across restart", { skip: process.platform !== "linux" }, async (t) => {
  ...
  const makeService = () => new TaskService(db, new Hub(), new ProcessSupervisor(1), new InProcessBackend(), new WorktreeManager(), new LocalAttachmentStorage(db), new LocalRepositoryPaths(), new LocalNativeSessionAdapter(db.path), new NodeIdentifierGenerator());
...
test("a first local import preserves cwd, waits for exit, and fails closed on transcript rewrites", { skip: process.platform !== "linux" }, async (t) => {
  ...
  const db = new Db(join(f.dir, "state/palmagent.db"));
  f.defer(() => db.close());
  const service = new TaskService(db, new Hub(), new ProcessSupervisor(1), new InProcessBackend(), new WorktreeManager(), new LocalAttachmentStorage(db), new LocalRepositoryPaths(), new LocalNativeSessionAdapter(db.path), new NodeIdentifierGenerator());
  await service.init();
  const writer = spawn(process.execPath, ["-e", "setInterval(() => {}, 1000)"], { argv0: "codex", stdio: "ignore" });
  await once(writer, "spawn");
  f.defer(() => stopWriter(writer));
