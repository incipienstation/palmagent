...
export function terminalRoutes(deps: Pick<HttpDependencies, "auth" | "terminals" | "terminalTickets" | "config">) {
  ...
  app.use("*", async (c, next) => {
    if (!deps.auth.enabled) throw new ApplicationError("forbidden", "Sign in must be enabled for shell access");
    if (c.req.method !== "GET" && c.req.header("origin") !== deps.auth.origin) throw new ApplicationError("forbidden", "Same-origin request required");
    await next();
  });
  const service = () => { if (!deps.terminals) throw new ApplicationError("service_unavailable", "Terminals unavailable"); return deps.terminals; };
  return app
    ...
    .get("/:id", idParams, c => c.json({ terminal: service().getPublic(c.req.valid("param").id) }, 200))
    ...
    .post("/:id/attach-ticket", idParams, c => {
      ...
      if (!token || !deps.auth.sessionValid(token) || !deps.terminalTickets) throw new ApplicationError("unauthorized", "Authentication required");
